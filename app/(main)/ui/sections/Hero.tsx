...
export function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
      ...
      {/* Animated Glow Orbs */}
      <div className="absolute top-1/4 start-1/4 w-96 h-96 bg-mk-purple/20 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-1/4 end-1/4 w-80 h-80 bg-mk-cyan/15 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />

      {/* Hero-wide floating particles (DOM-based) */}
      <HeroParticles />

      <div className="container mx-auto px-4 relative z-10">
        {/* True 50/50 split on desktop */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-center h-[700px]">
          {/* Left Column: Text Content */}
          <motion.div
            initial={{ opacity: 0, x: -50, filter: 'blur(10px)' }}
            animate={showContent ? { opacity: 1, x: 0, filter: 'blur(0px)' } : { opacity: 0, x: -50, filter: 'blur(10px)' }}
            transition={{ duration: 0.8, ease: 'easeOut', delay: 0.2 }}
            className="flex flex-col justify-center space-y-6 text-center md:text-start relative z-10"
          >
            {/* Tagline Badge */}
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-secondary/80 backdrop-blur-sm border border-border w-fit mx-auto md:mx-0"
            >
              <Sparkles className="h-4 w-4 text-mk-purple" />
              <span className="text-sm font-medium text-muted-foreground">{t.hero.tagline}</span>
            </motion.div>
            ...
            >
              <Link
                href="/login"
                className="inline-flex items-center gap-2 btn-gradient text-lg px-10 py-4 glow-effect-strong"
              >
                {t.hero.cta}
          ...
          {/* Right Column: 3D Model - True 50% width, centered */}
          <motion.div
            initial={{ opacity: 0, x: 100, rotateY: -0.5 }}
            animate={showContent ? { opacity: 1, x: 0, rotateY: 0 } : { opacity: 0, x: 100, rotateY: -0.5 }}
            transition={{ duration: 0.8, ease: [0.25, 0.1, 0.25, 1], delay: 0.1 }}
            className="relative hidden md:flex md:items-center md:justify-center w-full h-[700px]"
          >
          ...
          {/* Mobile: 3D Model (reduced/hidden for performance) */}
          <div className="md:hidden relative h-64 opacity-50">
            <Suspense fallback={<Hero3DLoader isLoading={true} />}>
